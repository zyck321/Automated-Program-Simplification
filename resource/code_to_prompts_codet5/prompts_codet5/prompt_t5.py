...
from openprompt.prompts import PrefixTuningTemplate,SoftTemplate,ManualTemplate
...
def main(args):
	...
	#plm = T5ForConditionalGeneration.from_pretrained("Salesforce/codet5-base")
	#tokenizer = AutoTokenizer.from_pretrained("Salesforce/codet5p-220m")
	#plm = T5ForConditionalGeneration.from_pretrained("Salesforce/codet5p-220m")

	WrapperClass = T5TokenizerWrapper

	# define template
	# promptTemplate = SoftTemplate(model=plm, tokenizer=tokenizer, text='{"placeholder":"text_a"} {"mask"} ', num_tokens=50)
	promptTemplate = ManualTemplate(tokenizer=tokenizer, text='simplify the following java method: {"placeholder":"text_a"}, the simplified version is: {"mask"}')
...
def calculate_bleu(file_name, args, tokenizer, device, model, promptTemplate, WrapperClass, output_file_name=None, is_test=False, dev_dataloader=None,
	...
	for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
		batch = batch.to(device)
		with torch.no_grad():
			_, output_sentence = model.generate(batch, max_new_tokens=args.max_target_length, num_beams=args.beam_size,num_return_sequences=args.beam_size)
	...
	with open(os.path.join(args.output_dir, file_prefix + ".output"), 'w') as f, open(
			os.path.join(args.output_dir, file_prefix + ".gold"), 'w') as f1:
                ...
                for gold in groundtruth_sentence:
                    f1.write(gold.replace('\n',' ') + '\n')
                for ref in generated_texts:
                    f.write(ref.replace('\n',' ') + '\n')
